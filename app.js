...
updateUserLocation = function(data, socket_session_id){
	...
	var looop_in_that_location = connection.query('SELECT stat.id as looop_id, stat.status_type, stat.image_url, stat.created_at as posted_time, stat.user_id, user.name, stat.status, if((lik.status_id = stat.id and lik.user_id = '+data.user_id+'), true, false) AS like, if((fol.user_id = stat.user_id and fol.following_id = '+data.user_id+'), true, false) AS relationship, ( 3959 * acos( cos( radians('+data.latitude+') ) * cos( radians( stat.status_location_latitude ) ) * cos( radians( stat.status_location_longitude ) - radians('+data.longitude+') ) + sin( radians('+data.latitude+') ) * sin( radians( stat.status_location_latitude ) ) ) ) AS distance FROM status as stat LEFT JOIN following_mapping as fol on fol.user_id = stat.user_id LEFT JOIN likes as lik on lik.status_id = stat.id LEFT JOIN users as user on user.id = stat.user_id group by looop_id HAVING distance < 500');
